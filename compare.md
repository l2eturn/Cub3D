...
### macOS (project_ref_mac)
```c
// build_music_cmd — volume เป็น float 0.00–1.00
p1 = "(while :; do afplay -v ";
p2 = " ./resource/horror_game_music.mp3; done) >/dev/null 2>&1 & echo $! > /tmp/cub3d_music.pid";
// → "(while :; do afplay -v 0.30 ./resource/horror_game_music.mp3; done) ..."
...
**ปัญหา 1: `ffplay` ไม่ใช่ built-in บน macOS**
- Linux: `ffplay` มาจาก `ffmpeg` package ที่นิยมติดตั้งใน lab 42
- macOS: ต้องติดตั้งผ่าน `brew install ffmpeg` เอง — ไม่ guaranteed
...
Linux ใช้ `kill $(cat /tmp/cub3d_music.pid)` — kill `ffplay` process ตรงๆ ได้เพราะ PID ของ `ffplay` คือ process หลัก

macOS ต้องใช้ `pkill -f 'afplay.*horror_game_music'` เพราะ:
```
shell PID (ที่เก็บใน .pid)
└── while loop
    └── afplay  ← PID ต่างออกไป
```
ถ้า `kill` แค่ shell parent → `afplay` กลายเป็น orphan เล่นต่อจนจบเพลง  
...
| ไฟล์ | สิ่งที่เปลี่ยน |
|------|----------------|
| `Makefile` | เอา `-ldl -pthread` ออก, เพิ่ม `-framework Cocoa/OpenGL/IOKit`, เพิ่ม `GLFW_FLAGS` |
| `src/bonus/sound.c` | `aplay -q` → `afplay` |
| `src/bonus/audio.c` | `ffplay -loop 0 -volume X` → `(while :; do afplay -v 0.XX; done)`, volume scale ใหม่, `music_stop` → `pkill -f` |
