...
/*
** Audio note: aplay + system() is NOT 42-Norm safe (system() is on the
** forbidden function list).  Kept here for reference only — for a Norm-
** compliant submission, replace with BASS, miniaudio, or no audio.
**
** The "& >/dev/null 2>&1" suffix:
**   - "&" → fork into background → returns immediately
**   - redirects swallow aplay's chatter so the terminal stays clean
*/
